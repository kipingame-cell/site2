...
/** Личный вариант: большой цельный рассказ ~4000 знаков, размыто-конкретный. */
function exitPlusSingleHTML(m) {
  ...
  const paras = [
    ...
    `Начинается всё с центра — аркан ${p.center} (${a(p.center).name}). Это ваша розетка, состояние, в котором вы восстанавливаетесь быстрее всего. Когда вы живёте из центра — ${prof(p.center).adv ? prof(p.center).adv.toLowerCase() : ''} — остальные каналы наполняются сами, без насилия над собой. Если внутри штормит, не начинайте разбор с денег или отношений: возвращайтесь сюда. Спокойный центр тихо чинит половину матрицы без вашего участия.`,
    `Ваша визитка — аркан ${p.day} (${a(p.day).name}): так вас считывают люди в первые минуты, и так вы видите себя изнутри. Сверху, в канале талантов, стоит триада ${triadNames(pk.talents)} — вместе они дают ${prof(p.month).syn || 'ваш главный дар'}, ${prof(ax.top.mid).syn || 'особое мышление'} и ${prof(ax.top.inner).syn || 'свой способ проявляться'}. Дар не нужно «зарабатывать» — он уже работает каждый раз, когда вам легко и интересно. Лёгкость — это не обман и не «что-то не так», это маркер плюса: то, что даётся вам играючи, и есть ваша сильная сторона.`,
    `Денежный канал собран из энергий ${triadNames(pk.money)}. Здесь важен не размер усилий, а попадание в свою роль: ${a(ax.right.mid).money || a(p.year).money || ''} Точка «под долларом» — аркан ${dollar} (${a(dollar).name}) — показывает, что открывает поток: ${prof(dollar).adv ? prof(dollar).adv.toLowerCase() : ''}. А центр линии благополучия, аркан ${balance} (${a(balance).name}), напоминает: деньги и близость у вас питаются от одного источника. Поэтому, когда финансы «тормозят», это почти никогда не про деньги — проверьте, в каком положении кнопки этой триады.`,
    `Канал отношений — ${triadNames(pk.relations)}. В близость вы входите через ${prof(relIn).syn || 'свой особый способ'}, а человек, рядом с которым вам будет по-настоящему хорошо, описан арканом ${heart} (${a(heart).name}). Программа близости просит одного: ${prof(ax.bottom.mid).adv ? prof(ax.bottom.mid).adv.toLowerCase() : ''}. Если отношения раз за разом повторяют похожий сценарий — дело не в «не тех людях», а в этой триаде: она будет крутить один урок, пока не зазвучит в плюсе. Хорошая новость: стоит сценарию перевернуться, и типаж вокруг вас меняется сам.`,
    `Кармический хвост — ${m.karmicTail.map((n) => `${n} (${a(n).name})`).join(' — ')}: главный урок, с которым вы пришли. Это одновременно самое уязвимое и самое сильное место матрицы: то, что больше всего мешает в минусе, после проработки даёт больше всего силы. Сигнал, что хвост включился: ${prof(p.tail).minus || 'старые реакции на автомате'}. Выход простой и небыстрый: ${prof(p.tail).adv ? prof(p.tail).adv.toLowerCase() : ''}. Ничего «искупать» не нужно — достаточно замечать момент, когда включается старый сценарий, и в этот раз выбрать иначе. Каждый такой выбор переписывает программу.`,
    `Предназначение — не про профессию, а про состояние, которое вы приносите людям. Личное (20–40 лет) — аркан ${pr.personal} (${a(pr.personal).name}): ${prof(pr.personal).syn || 'ваша задача на первую половину пути'}. Социальное (40–60 лет) — аркан ${pr.social} (${a(pr.social).name}): ${prof(pr.social).syn || 'ваш вклад в мир'}. Общее — аркан ${pr.general} (${a(pr.general).name}), планетарное — ${pr.planetary} (${a(pr.planetary).name}). Не ждите, что оно «откроется» однажды: предназначение собирается из маленьких действий в своей теме каждый день, и в какой-то момент оглядываешься — а оно уже случилось.`,
...
function exitPlusCompatHTML(c) {
  ...
  const paras = [
    ...
    `Сердце вашей пары — центр, аркан ${p.center} (${a(p.center).name}). Это общая атмосфера союза: то, что вы чувствуете, просто находясь рядом. ${a(p.center).compat || ''} Когда центр в ресурсе, мелкие ссоры гаснут сами; когда он проседает — любая мелочь становится поводом. Поэтому главное вложение в отношения — не подарки и не разговоры «по душам» по расписанию, а состояние, в котором вы оба чаще всего находитесь.`,
    `Программа любви собрана из энергий ${triadNames(pk.relations)}. Это то, как вы сближаетесь, что является якорем союза и какой сценарий близости разворачивается между вами. В минусе эта триада даёт проверки на прочность: недопонимание, качели, ощущение «мы говорим на разных языках». В плюсе она же — главный цемент: ${prof(c.keys.relations).adv ? prof(c.keys.relations).adv.toLowerCase() : ''}. Практический ориентир прост: после хорошего дня вместе вспомните, что именно вы делали — и делайте это чаще, чем «выясняете отношения».`,
    `Деньги пары — триада ${triadNames(pk.money)}: через это союз зарабатывает и так открывается общий финансовый поток. Здесь почти всегда работает правило: деньги приходят не тогда, когда вы оба «пашете» сильнее, а когда каждый стоит в своей роли и не тянет одеяло. Если общий бюджет вечно становится полем битвы — посмотрите на эти три числа: одна из энергий проживается в минусе, и денежные споры — только её симптом, а не причина.`,
    ...
    `Кармическая задача пары — ${c.karmicTail.map((n) => `${n} (${a(n).name})`).join(' — ')}: урок, ради которого вы встретились. Он будет периодически напоминать о себе кризисами — это нормально и не значит, что «всё рушится». Кризис в вашей паре — это не стена, а дверь, которую хвост подсовывает снова и снова, пока вы не пройдёте её по-другому: вместе, а не друг против друга. Прожитая в плюсе, эта энергия становится самым прочным слоем вашего фундамента.`,
    `Отдельно — про быт и повседневность, энергия ${p.day} (${a(p.day).name}). Большинство пар разрушает не судьба, а рутина: кто моет посуду, кто решает, кто прав в мелочах. Ваша бытовая энергия в плюсе даёт ощущение «дома», куда хочется возвращаться; в минусе — накопление мелких обид, которые однажды выглядят как «разлюбил(а)». Простое правило: обиды короткие — проговаривайте сразу и без накопления; тепло — длинное, его не бывает «слишком много».`,
